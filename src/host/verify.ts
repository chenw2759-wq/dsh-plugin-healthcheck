...
import { resolveHomePatch, resolveInstallAnchor } from './env.ts'

/** The L1 finding factory. */
function l1Finding(code: string, severity: CheckFinding['severity'], message: string, evidence: string[] = []): CheckFinding {
 ...
 */
export async function checkConfigComposition(profile: string, home: string): Promise<CheckFinding[]> {
  ...
  try {
    ...
    findings.push(l1Finding(
      'config-compose-failed', 'error',
      `配置树组合失败：${error instanceof Error ? error.message : String(error)}`,
    ))
    return findings
  }
  ...
  findings.push(l1Finding(
    'config-compose-ok', 'info',
    `配置树组合成功：${composed.length} 行（bundle 层 ${findings.length > 0 ? '有告警' : '无告警'}）`,
  ))
  return findings
}
 ...
 */
export function runSmokeBoot(profile: string, home: string, timeoutMs = SMOKE_TIMEOUT_MS): Promise<SmokeResult> {
  ...
  return new Promise((resolvePromise) => {
    ...
    let stderr = ''
    let settled = false
    const finish = (ok: boolean, stage?: string, error?: string): void => {
      if (settled) return
      settled = true
...
function writeSmokeOverlay(home: string, profile: string): string {
...
export function cleanupSmokeOverlay(home: string, profile: string): void {
  const path = join(home, 'profiles', profile, `.healthcheck-overlay-${process.pid}.yml`)
  try {
    unlinkSync(path)
  } catch {
