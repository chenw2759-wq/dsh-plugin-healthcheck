...
/** A single cordis-usage hit. */
interface CordisHit {
  code: 'async-plugin-then-sync' | 'new-service-no-config' | 'inject-missing-service'
  file: string
  line: number
  snippet: string
  detail: string
...
/** The inject declaration of one plugin (name → array of service keys). */
...
function readInject(root: string): string[] {
 ...
 */
function detectAsyncPluginThenSync(root: string): CordisHit[] {
  const hits: CordisHit[] = []
 ...
 * "reading 'xxx'"。
 */
function detectNewServiceNoConfig(root: string): CordisHit[] {
  const hits: CordisHit[] = []
  for (const file of collectFiles(root)) {
    let content: string
    try {
 ...
 * 注释、声明行，降低误报。
 */
function detectInjectMissing(root: string, info: PluginInfo): CordisHit[] {
  const hits: CordisHit[] = []
  for (const file of collectFiles(root)) {
    let content: string
    try {
 ...
 */
export function checkCordisUsage(root: string, pluginName: string): CheckFinding[] {
  if (root === '') return []
  const info: PluginInfo = { root, inject: readInject(root) }
  const hits: CordisHit[] = [
