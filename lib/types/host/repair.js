...
import { resolveHome, resolveHomePatch, resolveInstallRoot } from "./env.js";
/**
 ...
 */
export function assertSafeTarget(path, home = resolveHome()) {
    if (typeof path !== 'string' || path === '')
        throw repairError('bad-target', 'empty repair path');
...
export function applyRepair(action, home = resolveHome()) {
    switch (action.kind) {
        case 'files-whitelist': {
            if (action.path === undefined)
                return { action: 'files-whitelist', applied: false, message: 'missing target path' };
            return repairFilesWhitelist(action.path, home);
        }
        case 'remove-copies': {
            if (action.path === undefined)
                return { action: 'remove-copies', applied: false, message: 'missing target path' };
...
function repairFilesWhitelist(path, home) {
 ...
 */
export function rollbackPlugin(pluginId, home = resolveHome()) {
...
export function undoRollback(pluginId, home = resolveHome()) {
    const target = assertSafeTarget(resolveHomePatch(home), home);
    const content = readFileSync(target, 'utf8');
...
export function appendHistory(record, home = resolveHome(), keep = 20) {
...
export function readHistory(home = resolveHome()) {
...
export function findPluginRow(rows, name) {
