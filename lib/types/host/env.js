 ...
 */
export function resolveInstallRoot(home = resolveHome()) {
 ...
 */
export function listProfilePlugins(profile, home = resolveHome()) {
    ...
    for (const [name, spec] of Object.entries(manifest.dependencies ?? {})) {
        ...
        const row = { name, spec, bundle: bundles.has(name), installedDir };
 ...
 */
export function collectPluginRowIds(row, profileDir) {
    const ids = new Set();
    ids.add(row.name);
    const roots = [row.sourceDir, row.installedDir].filter((root) => root !== undefined && root !== '');
 ...
 */
export function collectDisabledRows(profile, home = resolveHome()) {
 ...
 */
export function listBuiltinBundles(home = resolveHome()) {
