...
export function HealthcheckSection(props) {
    ...
    const [inventory, setInventory] = useState({ profile: [], builtin: [], counts: { profile: 0, builtin: 0, total: 0 } });
    ...
    return (_jsxs("div", { className: css.root, children: [_jsx("p", { className: css.description, children: t('description') }), _jsx("p", { className: css.ironRule, children: t('ironRule') }), _jsx("p", { className: css.ironRule, children: t('malwareIronRule') }), _jsxs("div", { className: css.controls, children: [_jsxs("select", { className: css.scope, value: scopePlugin, onChange: (event) => { setScopePlugin(event.target.value); }, "aria-label": t('scopePlugin'), children: [_jsxs("option", { value: "", children: [t('scopeAll'), " (", inventory.counts.total, ")"] }), inventory.profile.length > 0
                                ? (_jsx("optgroup", { label: `${t('scopeProfile')} (${inventory.counts.profile})`, children: inventory.profile.map((row) => (_jsxs("option", { value: row.name, children: [row.disabled ? `[${t('disabledBadge')}] ` : '', row.name] }, row.name))) }))
                                : null, inventory.builtin.length > 0
                                ? (_jsx("optgroup", { label: `${t('scopeBuiltin')} (${inventory.counts.builtin})`, children: inventory.builtin.map((row) => (_jsx("option", { value: row.name, children: row.name }, row.name))) }))
