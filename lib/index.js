...
*/
function listBuiltinBundles(home = resolveHome()) {
...
function checkDependencySpec(ctx, row) {
	...
	if (row.spec.startsWith("file:") && peers.some((p) => HIGH_RISK_PACKAGES.some((h) => p === `@deepseek-ai/${h}`))) {
		...
		if (autoInstallPeersEnabled(ctx.profileDir)) findings.push({
			layer: "l0",
...
const CONFIG_REQUIRED_SERVICES = ["SandboxedFileSystem"];
/** File extensions scanned. */
const SCAN_EXT$1 = /\.(ts|tsx|js|mjs|cjs)$/i;
/** Directories skipped. */
...
const CORE_MEMBERS = /* @__PURE__ */ new Set([
	"plugin",
	"isolate",
	"effect",
	"provide",
...
*/
const SERVICE_LIKE_PREFIX = /* @__PURE__ */ new Set([
	"fs",
	"shell",
	"session",
	"storage",
...
function readInject(root) {
	...
	const files = collectFiles$1(root);
	for (const file of files) {
		...
		if (match === null) continue;
		for (const key of match[1].matchAll(/['"]([^'"]+)['"]/g)) inject.add(key[1]);
	}
	return [...inject];
}
/** Walk the plugin tree for scannable source files. */
function collectFiles$1(root) {
	const files = [];
	const walk = (dir, depth) => {
		...
		for (const entry of entries) {
			const path = join(dir, entry.name);
			if (entry.isDirectory()) {
				if (!SKIP_DIRS$1.has(entry.name)) walk(path, depth + 1);
				continue;
			}
			if (SCAN_EXT$1.test(entry.name)) files.push(path);
		}
	};
	walk(root, 0);
	return files;
...
function detectAsyncPluginThenSync(root) {
	const hits = [];
	for (const file of collectFiles$1(root)) {
		let content;
		try {
			content = readFileSync(file, "utf8");
		} catch {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (/(?<!await\s)(?:ctx|localCtx|hostCtx|scope)\.plugin\s*\(/.exec(codeLine) === null) continue;
			for (let j = i + 1; j <= i + 6 && j < lines.length; j++) {
				const ahead = stripLineComment(lines[j]);
				if (ahead === "") continue;
				if (/\bawait\b/.test(ahead)) break;
	...
	for (const file of collectFiles$1(root)) {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			for (const service of CONFIG_REQUIRED_SERVICES) {
				...
				if (match === null) continue;
				if (codeLine.slice(match.index + match[0].length).split(/,(?![^()]*\()/).length <= 1) hits.push({
...
function detectInjectMissing(root, info) {
	...
	for (const file of collectFiles$1(root)) {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (CORE_MEMBERS.has(service) || info.inject.includes(service)) continue;
			if (![...SERVICE_LIKE_PREFIX].some((prefix) => service === prefix || service.startsWith(prefix + ":"))) continue;
			hits.push({
...
function checkCordisUsage(root, pluginName) {
	...
	const findings = [];
	for (const hit of hits) findings.push({
		layer: "l0",
		code: hit.code,
		severity: "warn",
		plugin: pluginName,
...
async function executeRun(state, pluginFilter) {
	try {
		...
		let scoped = pluginFilter !== void 0 ? rows.filter((row) => row.name === pluginFilter) : rows;
		if (pluginFilter !== void 0 && scoped.length === 0) scoped = listBuiltinBundles(state.home).filter((row) => row.name === pluginFilter);
		if (state.layers.includes("l0")) {
			...
			for (const row of scoped) {
				for (const checker of perPlugin) state.findings.push(...checker(ctx, row));
		...
		if (pathname === "/healthcheck/inventory" && req.method === "GET") {
			...
			json(res, okEnvelope({
				...
				counts: {
					...
					total: profileRows.length + builtin.length
				}
			}));
			return;
		}
